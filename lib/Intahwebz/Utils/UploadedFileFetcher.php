...
class UploadedFileFetcher implements \Intahwebz\FileFetcher {
    ...
    function getUploadedFile($formFileName)
    {
        ...
        if(isset($files[$formFileName]) == FALSE) {
            throw new \Intahwebz\FileUploadException("File not uploaded. \$files[".$formFileName."] is not set.");
        }
        else{
            if($files[$formFileName]['error'] == UPLOAD_ERR_OK) {
                if(is_uploaded_file($files[$formFileName]['tmp_name'])) {
                    ...
                    throw new \Intahwebz\FileUploadException("File not uploaded. Status [".$files[$formFileName]['error']."] indicated error.");
                }
            }
            else{
                throw new \Intahwebz\FileUploadException("Error detected in upload: ".getFileUploadErrorMeaning($files[$formFileName]['error']));
            }
        }
    }  
}
